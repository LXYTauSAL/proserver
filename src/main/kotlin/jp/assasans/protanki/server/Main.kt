...
class SocketServer : ISocketServer {
    ...
    override val players: MutableList<UserSocket> = CopyOnWriteArrayList()
    ...
    override suspend fun run(scope: CoroutineScope) {
        clientScope = CoroutineScope(scope.coroutineContext + SupervisorJob())
...
//                    coroutineScope.launch { socket.handle() }
                    clientScope.launch { socket.handle() }
...
//        withContext(Dispatchers.IO) { server.close() }
        ...
        for (player in players.toList()) {
            try {
                ...
                logger.warn(exception) { "Failed to deactivate player ${'$'}player" }
            }
        }
                ...
                Moshi.Builder()
                    .add(
                        PolymorphicJsonAdapterFactory.of(ProcessMessage::class.java, "_").let {
                            ...
                            reflections.get(Scanners.SubTypes.of(ProcessMessage::class.java).asClass<ProcessMessage>())
                                .forEach { type ->
                                    val messageType = type.kotlin.cast<KClass<ProcessMessage>>()
                                    val name = messageType.simpleName
                                        ?: throw IllegalStateException("$messageType has no simple name")
