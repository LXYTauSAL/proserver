...
class ResourceServer(
    private val upstreamBaseUrl: String = System.getenv("UPSTREAM_BASE_URL") ?: "http://54.36.175.134"
) : IResourceServer, KoinComponent {
        ...
        engine = embeddedServer(Netty, port = 8080, host = "0.0.0.0") {
            routing {
                get("/{id1}/{id2}/{id3}/{id4}/{version}/{file}") {
                    ...
                    val resource =
                        resourceManager.get("static/$originalPackName/${resourceId.id}/${resourceId.version}/$file")
...
//        expectSuccess = false
            val encodedId = ResourceUtils.encodeId(resourceId)

            val response = runCatching {
                client.get {
                    expectSuccess = false
                    url {
                        takeFrom(upstreamBaseUrl)
                        appendPathSegments(*encodedId.toTypedArray(), file)
                    }
                }
            }.getOrElse { exception ->
                logger.warn(exception) { "Failed to download resource ${resourceId.id}:${resourceId.version}/$file" }
...
//      throw Exception("Failed to download resource ${resourceId.id}:${resourceId.version}/${file}. Status code: ${response.status}")
            logger.warn { "Upstream returned ${response.status} for resource ${resourceId.id}:${resourceId.version}/$file" }
            null
        }
    }
