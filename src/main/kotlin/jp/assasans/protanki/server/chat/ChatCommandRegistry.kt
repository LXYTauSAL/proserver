...
abstract class CommandParseResult(val inputArguments: ParsedArguments) {
    class Success(inputArguments: ParsedArguments, val parsedCommand: ParsedCommand) :
...
class ChatCommandRegistry : IChatCommandRegistry, KoinComponent {
    ...
    override fun parseCommand(parsedArguments: ParsedArguments): CommandParseResult {
            ...
            if (canParseSubcommands && !argument.isQuoted) {
                if (command.subcommands.isNotEmpty()) {
                    val subcommand = command.subcommands.find { subcommand ->
                        ...
                        if (subcommand.aliases.any { alias ->
                                alias.equals(
                                    argument.value,
                                    ignoreCase = true
                                )
        ...
        if (missingArguments.isNotEmpty()) return CommandParseResult.TooFewArguments(
            parsedArguments,
            command,
            missingArguments
        )
    ...
    override suspend fun callCommand(socket: UserSocket, parsed: ParsedCommand, source: CommandInvocationSource) {
        val handler = parsed.command.handler
        if (handler == null) {
            // 可以根据 source 决定走 battle chat 还是 lobby chat，这里简化示意：
            socket.sendBattleChat("Command ${parsed.command.name} has no default action. Try one of: ${
                parsed.command.subcommands.joinToString { it.name }
            }")
            return
        }
...
class ParsedCommandArguments(
    ...
    operator fun <T : Any> get(name: String): T =
        getOrNull(name) ?: throw IllegalArgumentException("No argument with name $name")
