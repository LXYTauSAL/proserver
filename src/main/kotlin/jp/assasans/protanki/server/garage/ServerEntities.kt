...
class ServerGarageItemWeaponModification(
  ...
  @Json val damage: WeaponDamage = WeaponDamage(
    discrete = WeaponDamage.Discrete(),
    fixed = WeaponDamage.Fixed(21.12)
  ),

  // ✅ 这里改用工厂函数，根据 JSON 算出转速和“几乎瞬间”的加速度
  override val physics: WeaponPhysics = createWeaponPhysics(properties),

  @Json val visual: WeaponVisual?
) : ServerGarageItemModification(previewResourceId, object3ds, rank, price, properties, physics) {

  companion object {
    private fun createWeaponPhysics(properties: List<ServerGarageItemProperty>): WeaponPhysics {
      val speedRad = getTurretRotationSpeed(properties)

      // 理论上想“没有过渡”，就是加速度 → ∞
      // 实际我们给一个足够大的值，比如最大速度的 1000 倍
      val accel = speedRad * 1.2

      // 这里顺便也可以从 JSON 里读 IMPACT_FORCE，你懒得搞的话就先保持之前的常量 1.75 也行
      val impactForce = getImpactForce(properties)

      return WeaponPhysics(
        turretRotationSpeed = speedRad,
        turretTurnAcceleration = accel,
        impactForce = impactForce,
        kickback = 1.4043
      )
    }

    private fun getTurretRotationSpeed(properties: List<ServerGarageItemProperty>): Double {
      val prop = properties.firstOrNull { it.property == "TURRET_TURN_SPEED" }
      val raw = prop?.value
      ...
      return deg / 180.0 * Math.PI // 度 -> 弧度
    }

    private fun getImpactForce(properties: List<ServerGarageItemProperty>): Double {
      val prop = properties.firstOrNull { it.property == "IMPACT_FORCE" }
      val raw = prop?.value
