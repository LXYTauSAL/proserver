    ...
    suspend fun run() {
        ...
        chatCommandRegistry.apply {
            ...
            command("property") {
                ...
                description("Get or set battle properties")

                // 新增：顶层 handler，避免 /property 直接报错
                handler {
                    reply("Usage: /property <list|get|set> ...")
                }
            ...
            command("bonus") {
                ...
                subcommand("spawn") {
                    ...
                    handler {
                        ...
                        val position =
                            Random.nextVector3(bonusPoint.position.min.toVector(), bonusPoint.position.max.toVector())
                        val rotation = Quaternion()
                        rotation.fromEulerAngles(bonusPoint.rotation.toVector())

                        val bonus = when (bonusType) {
                            BonusType.Health -> BattleRepairKitBonus(
                                battle,
                                battle.bonusProcessor.nextId,
                                position,
                                rotation
                            )
                            BonusType.DoubleArmor -> BattleDoubleArmorBonus(
                                battle,
                                battle.bonusProcessor.nextId,
                                position,
                                rotation
                            )
                            BonusType.DoubleDamage -> BattleDoubleDamageBonus(
                                battle,
                                battle.bonusProcessor.nextId,
                                position,
                                rotation
                            )
                            BonusType.Nitro -> BattleNitroBonus(
                                battle,
                                battle.bonusProcessor.nextId,
                                position,
                                rotation
            ...
            command("addxp") {
                ...
                handler {
                    val amountRaw: String = arguments.get<String>("amount").trim()
                    val amountLong = amountRaw.toLongOrNull()
                    if (amountLong == null) {
                        reply("Invalid amount: '$amountRaw'")
                        return@handler
                    }
                    if (amountLong !in Int.MIN_VALUE.toLong()..Int.MAX_VALUE.toLong()) {
                        reply("Amount is out of range: '$amountRaw'")
                        return@handler
                    }
                    val amount = amountLong.toInt()

                    val username: String? = arguments.getOrNull("user")
                    val player =
                        if (username != null) socketServer.players.find { it.user?.username == username } else socket
                    if (player == null) {
                        reply("Player not found: $username")
                        return@handler
                    }
            ...
            command("addfund") {
                ...
                handler {
                    ...
                    val battleId: String? = arguments.getOrNull("battle")
                    val battle = if (battleId != null) {
                        battleProcessor.battles.singleOrNull { it.id == battleId }
                    } else {
                        socket.battle
                    }
            ...
            command("invite") {
                ...
                handler {
                    reply("Usage: /invite <toggle|add|delete|list> ...")
                }
