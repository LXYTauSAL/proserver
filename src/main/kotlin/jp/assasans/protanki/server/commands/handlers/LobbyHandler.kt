...
class LobbyHandler : ICommandHandler, KoinComponent {
  ...
  suspend fun createBattle(socket: UserSocket, data: BattleCreateData) {
    ...
    if(data.parkourMode) {
      ...
      battle.properties[BattleProperty.DamageEnabled] = true
    ...
    battle.properties[BattleProperty.ScoreLimit] = data.scoreLimit   // ★ 新增

    // DM 模式：如果设置了 time_limit，就从房间创建时开始算局内时间
    battle.startTimeLimitJobIfNeeded()
